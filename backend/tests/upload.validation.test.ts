...
assert.equal(validateStoredAvatarValue(`data:image/png;base64,${largerPng.toString('base64')}`).valid, true)
...
assert.equal(validateStoredAvatarValue('/api/uploads/files/avatar.png').valid, true)
assert.equal(validateStoredAvatarValue('JD').valid, true)
assert.equal(validateStoredAvatarValue('').valid, true)
assert.equal(validateStoredAvatarValue(42).valid, false)
assert.equal(validateStoredAvatarValue('javascript:alert(1)').valid, false)
assert.equal(validateStoredAvatarValue('https://example.test/avatar image.png').valid, false)
assert.equal(validateStoredAvatarValue(`data:text/html;base64,${Buffer.from('<script />').toString('base64')}`).valid, false)
