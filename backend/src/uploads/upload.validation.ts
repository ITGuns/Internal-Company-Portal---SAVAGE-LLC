...
interface AvatarValueValidationResult {
    valid: boolean
    value?: string
...
export function validateAvatarValue(value: unknown): AvatarValueValidationResult {
    ...
    if (avatar.startsWith('data:')) {
        ...
        return { valid: true, value: avatar, type }
    }
    ...
    return { valid: false, error: 'Avatar must be an http(s) URL, relative path, or supported image data URI' }
}
