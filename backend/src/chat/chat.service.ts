...
import type { ChatArchiveView } from './chat.limits'
...
export class ChatService {
    ...
    async findDirectConversation(user1Id: string, user2Id: string) {
        ...
        const conversation = conversations.find(c => c.participants.length === 2 &&
     ...
     */
    async getUserConversations(userId: string, archiveView: ChatArchiveView = 'active') {
        const archiveFilter = archiveView === 'all'
            ? {}
        ...
        return conversations.map((conv) => {
            const currentParticipant = conv.participants.find((participant) => participant.userId === userId)
   ...
   */
    ...
    async setConversationArchived(conversationId: string, userId: string, archived: boolean): Promise<Participant | null> {
    ...
    async toggleReaction(messageId: string, userId: string, emoji: string) {
        ...
        const isParticipant = await this.isParticipant(message.conversationId, userId)
        ...
        if (existing) {
            ...
            return {
                ...
                userId,
                active: false,
                reactions: await this.getMessageReactions(messageId),
            }
        }
