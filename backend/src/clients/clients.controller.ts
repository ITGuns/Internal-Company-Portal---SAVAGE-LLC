...
import {
  ...
  serializeClientStorageRootForManagement,
...
export class ClientsController {
  ...
  router(): Router {
    ...
    router.patch('/organizations/:id/storage-root', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        if (!canManageClientOrganization(access)) {
          return res.status(403).json({ error: 'Only operations managers and admins can update client storage' })
        }

        const organizationId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        const storageRoot = await this.service.upsertStorageRoot(
          organizationId,
          parseUpsertClientStorageRootInput(req.body || {}),
          access.requesterId,
        )
        res.json(serializeClientStorageRootForManagement(storageRoot))
      } catch (error) {
        if (error instanceof ClientValidationError) {
          return res.status(400).json({ error: error.message })
        }

        logger.error('[Clients] Error updating storage root:', error)
        res.status(500).json({ error: 'Failed to update client storage root' })
      }
    })

    router.post('/organizations/:id/booking-requests', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        const organizationId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        if (!canManageClientOrganization(access) && !canReadClientOrganization(access, { id: organizationId })) {
          return res.status(403).json({ error: 'You can only request calls for your assigned client organization' })
        }

        const input = parseCreateClientBookingRequestInput(req.body || {})
        const booking = await this.service.createBookingRequest(
          organizationId,
          access.requesterId,
          access.isPrivileged
            ? input
        ...
        res.status(201).json(
          access.isPrivileged
            ? serializeClientBookingRequestForManagement(booking)
            : serializeClientBookingRequestForClient(booking),
        )
      } catch (error) {
        if (error instanceof ClientValidationError) {
          return res.status(400).json({ error: error.message })
        ...
        logger.error('[Clients] Error creating booking request:', error)
        res.status(500).json({ error: 'Failed to create booking request' })
      }
    })

    router.patch('/booking-requests/:id', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        if (!canManageClientOrganization(access)) {
          return res.status(403).json({ error: 'Only operations managers and admins can update booking requests' })
        }

        const bookingId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        const booking = await this.service.findBookingRequestById(bookingId)
        if (!booking) return res.status(404).json({ error: 'Booking request not found' })

        const updatedBooking = await this.service.updateBookingRequest(
          bookingId,
          parseUpdateClientBookingRequestInput(req.body || {}),
          access.requesterId,
        )
        res.json(serializeClientBookingRequestForManagement(updatedBooking))
      } catch (error) {
        if (error instanceof ClientValidationError) {
          return res.status(400).json({ error: error.message })
        }

        logger.error('[Clients] Error updating booking request:', error)
        res.status(500).json({ error: 'Failed to update booking request' })
      }
    })

    router.patch('/organizations/:id/payment-connections', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        if (!canManageClientOrganization(access)) {
          return res.status(403).json({ error: 'Only operations managers and admins can update payment connections' })
        }

        const organizationId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        const connection = await this.service.upsertPaymentConnection(
          organizationId,
          parseUpsertClientPaymentConnectionInput(req.body || {}),
          access.requesterId,
        )
        res.json(serializeClientPaymentConnectionForManagement(connection))
      } catch (error) {
        if (error instanceof ClientValidationError) {
          return res.status(400).json({ error: error.message })
        }
        ...
        logger.error('[Clients] Error updating payment connection:', error)
        res.status(500).json({ error: 'Failed to update payment connection' })
      }
    })

    router.post('/organizations/:id/invoices', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        if (!canManageClientOrganization(access)) {
          return res.status(403).json({ error: 'Only operations managers and admins can create invoices' })
        }

        const organizationId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        const invoice = await this.service.createInvoice(
          organizationId,
          parseCreateClientInvoiceInput(req.body || {}),
          access.requesterId,
        )
        res.status(201).json(serializeClientInvoiceForManagement(invoice))
      } catch (error) {
        if (error instanceof ClientValidationError) {
          return res.status(400).json({ error: error.message })
        }
        ...
        logger.error('[Clients] Error creating invoice:', error)
        res.status(500).json({ error: 'Failed to create invoice' })
      }
    })

    router.post('/organizations/:id/invoices/generate', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        if (!canManageClientOrganization(access)) {
          return res.status(403).json({ error: 'Only operations managers and admins can generate invoices' })
        }

        const organizationId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        const invoice = await this.service.generateMonthlyInvoice(
          organizationId,
          parseGenerateClientInvoiceInput(req.body || {}),
          access.requesterId,
        )
        res.status(201).json(serializeClientInvoiceForManagement(invoice))
      } catch (error) {
        ...
        logger.error('[Clients] Error generating invoice:', error)
        res.status(500).json({ error: 'Failed to generate invoice' })
      }
    })

    router.patch('/invoices/:id', authenticateToken, async (req: Request, res: Response) => {
      try {
        const access = await this.getAccessContext(req)
        if (!access) return res.status(401).json({ error: 'Authentication required' })
        if (!canManageClientOrganization(access)) {
          return res.status(403).json({ error: 'Only operations managers and admins can update invoices' })
        }

        const invoiceId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id
        const invoice = await this.service.findInvoiceById(invoiceId)
        if (!invoice) return res.status(404).json({ error: 'Invoice not found' })

        const updatedInvoice = await this.service.updateInvoice(
          invoiceId,
          parseUpdateClientInvoiceInput(req.body || {}),
          access.requesterId,
        )
        res.json(serializeClientInvoiceForManagement(updatedInvoice))
      } catch (error) {
        if (error instanceof ClientValidationError) {
          return res.status(400).json({ error: error.message })
        }
        ...
        logger.error('[Clients] Error updating invoice:', error)
    ...
    router.patch('/tickets/:id', authenticateToken, async (req: Request, res: Response) => {
      try {
        ...
        const updatedTicket = await this.service.updateTicket(
          ticketId,
          parseUpdateClientTicketInput(req.body || {}, canManageClientOrganization(access)),
          access.requesterId,
        )
