...
import { ClientProviderWorkflowsService } from './client-provider-workflows.service'
...
export class ClientsService {
  ...
  private providerWorkflows: ClientProviderWorkflowsService
  ...
  constructor() {
    ...
    this.providerWorkflows = new ClientProviderWorkflowsService(this.prisma)
  }
  ...
  async findOrganizationOverview(id: string, clientVisibleOnly: boolean) {
    return this.prisma.clientOrganization.findUnique({
      ...
      include: {
        ...
        paymentConnections: {
          orderBy: [{ provider: 'asc' }, { accountType: 'asc' }],
          take: 25,
        },
        invoices: {
          where: clientVisibleOnly ? { visibleToClient: true } : undefined,
  ...
  async updateTicket(ticketId: string, data: UpdateClientTicketInput, actorId?: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!previous) throw new ClientValidationError('Client ticket not found')
      ...
      if (data.assignedToId) {
        ...
        if (!assignee) {
          throw new ClientValidationError('Assigned staff member was not found')
        }
      }
      ...
      const hasClientVisibleUpdates = ['title', 'description', 'category', 'priority'].some((key) => key in data)
      await createClientActivity(tx, {
        organizationId: previous.organizationId,
        actorId,
        ...
        subjectType: 'ticket',
        subjectId: ticket.id,
        visibility: hasClientVisibleUpdates ? 'client' : 'internal',
        title: hasClientVisibleUpdates
          ? `Request updated: ${ticket.title}`
          : `Request triage updated: ${ticket.title}`,
        body: hasClientVisibleUpdates ? ticket.description || null : null,
        metadata: {
  ...
  async upsertStorageRoot(organizationId: string, data: UpsertClientStorageRootInput, actorId?: string) {
    return this.providerWorkflows.upsertStorageRoot(organizationId, data, actorId)
  }

  async createBookingRequest(organizationId: string, requestedById: string | undefined, data: CreateClientBookingRequestInput) {
    return this.providerWorkflows.createBookingRequest(organizationId, requestedById, data)
  }

  async findBookingRequestById(id: string) {
    return this.providerWorkflows.findBookingRequestById(id)
  }

  async updateBookingRequest(id: string, data: UpdateClientBookingRequestInput, actorId?: string) {
    return this.providerWorkflows.updateBookingRequest(id, data, actorId)
  }

  async upsertPaymentConnection(organizationId: string, data: UpsertClientPaymentConnectionInput, actorId?: string) {
    return this.providerWorkflows.upsertPaymentConnection(organizationId, data, actorId)
  }

  async createInvoice(organizationId: string, data: CreateClientInvoiceInput, actorId?: string) {
    return this.providerWorkflows.createInvoice(organizationId, data, actorId)
  }

  async generateMonthlyInvoice(organizationId: string, data: GenerateClientInvoiceInput, actorId?: string) {
    return this.providerWorkflows.generateMonthlyInvoice(organizationId, data, actorId)
  }

  async findInvoiceById(id: string) {
    return this.providerWorkflows.findInvoiceById(id)
  }

  async updateInvoice(id: string, data: UpdateClientInvoiceInput, actorId?: string) {
    return this.providerWorkflows.updateInvoice(id, data, actorId)
  }
