...
interface ClientStorageRootLike {
  id: string
  organizationId: string
  provider: string
  status: string
...
interface ClientBookingRequestLike {
  id: string
  organizationId: string
  requestedById?: string | null
  provider: string
  status: string
  subject: string
  preferredStartAt?: SerializableDate
  preferredEndAt?: SerializableDate
  timezone: string
...
interface ClientPaymentConnectionLike {
  id: string
  organizationId: string
  provider: string
  accountType: string
...
export function serializeClientStorageRootForClient(storageRoot: ClientStorageRootLike) {
  return {
    id: storageRoot.id,
    organizationId: storageRoot.organizationId,
    provider: storageRoot.provider,
    status: storageRoot.status,
    folderName: storageRoot.folderName,
    externalUrl: storageRoot.externalUrl || null,
    createdAt: serializeDate(storageRoot.createdAt),
    updatedAt: serializeDate(storageRoot.updatedAt),
  }
}

export function serializeClientStorageRootForManagement(storageRoot: ClientStorageRootLike) {
  return {
    ...serializeClientStorageRootForClient(storageRoot),
    directoryFolderId: storageRoot.directoryFolderId || null,
    externalFolderId: storageRoot.externalFolderId || null,
    notes: storageRoot.notes || null,
  }
}

export function serializeClientBookingRequestForClient(booking: ClientBookingRequestLike) {
  if (!booking.visibleToClient) return null
  ...
  return {
    ...
    preferredStartAt: serializeDate(booking.preferredStartAt),
    preferredEndAt: serializeDate(booking.preferredEndAt),
    timezone: booking.timezone,
    meetingUrl: booking.meetingUrl || null,
    visibleToClient: booking.visibleToClient,
    createdAt: serializeDate(booking.createdAt),
    updatedAt: serializeDate(booking.updatedAt),
  }
}

export function serializeClientBookingRequestForManagement(booking: ClientBookingRequestLike) {
  return {
    ...serializeClientBookingRequestForClient({ ...booking, visibleToClient: true }),
    requestedById: booking.requestedById || null,
    notes: booking.notes || null,
    visibleToClient: booking.visibleToClient,
  }
}

export function serializeClientPaymentConnectionForManagement(connection: ClientPaymentConnectionLike) {
  return {
    id: connection.id,
    ...
    externalCustomerId: connection.externalCustomerId || null,
    externalMerchantId: connection.externalMerchantId || null,
    lastFour: connection.lastFour || null,
...
export function serializeClientInvoiceForClient(invoice: ClientInvoiceLike) {
  if (!invoice.visibleToClient) return null
  ...
  return {
    ...
    issueAt: serializeDate(invoice.issueAt),
    dueAt: serializeDate(invoice.dueAt),
    paidAt: serializeDate(invoice.paidAt),
    hostedInvoiceUrl: invoice.hostedInvoiceUrl || null,
    visibleToClient: invoice.visibleToClient,
    createdAt: serializeDate(invoice.createdAt),
    updatedAt: serializeDate(invoice.updatedAt),
  }
}

export function serializeClientInvoiceForManagement(invoice: ClientInvoiceLike) {
  return {
    ...serializeClientInvoiceForClient({ ...invoice, visibleToClient: true }),
    billingStatusId: invoice.billingStatusId || null,
