...
import {
  ...
  filterPayslipsForReport,
  getUniquePayslipDepartments,
  summarizePayslipCosts,
  type PayrollReportDepartmentSummary,
...
function PayslipArchive() {
  ...
  const filtered = useMemo(() => filterPayslipsForReport(allPayslips, {
    query: search,
    department: filterDepartment,
    status: filterStatus,
  }), [allPayslips, search, filterDepartment, filterStatus]);

  const availableDepartments = useMemo(() => getUniquePayslipDepartments(allPayslips), [allPayslips]);
  const filteredSummary = useMemo(() => summarizePayslipCosts(filtered), [filtered]);
  const departmentSummary = useMemo(() => buildDepartmentCostSummary(filtered), [filtered]);
  ...
  const handleExportCsv = () => {
    downloadCsv("deskii-payslip-archive.csv", buildPayslipArchiveCsv(filtered));
    showToastSuccess(`Exported ${filtered.length} payslip${filtered.length === 1 ? "" : "s"}`);
  };
  ...
  const handleExportBatch = () => {
    ...
    downloadZip("deskii-payroll-report-batch.zip", buildReportZipArchive(files));
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={<PhilippinePeso className="w-4 h-4" />} label="Filtered Gross" value={`PHP ${filteredSummary.gross.toLocaleString()}`} bgColor="bg-blue-500" />
        <StatCard icon={<TrendingUp className="w-4 h-4" />} label="Filtered Net" value={`PHP ${filteredSummary.net.toLocaleString()}`} bgColor="bg-emerald-500" />
        <StatCard icon={<TrendingDown className="w-4 h-4" />} label="Filtered Deductions" value={`PHP ${filteredSummary.deductions.toLocaleString()}`} bgColor="bg-red-500" />
        <StatCard icon={<FileClock className="w-4 h-4" />} label="Filtered Hours" value={filteredSummary.hours.toFixed(1)} bgColor="bg-indigo-500" />
      </div>

      {departmentSummary.length > 0 && (
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-[var(--border)] flex items-center justify-between gap-3">
            <h4 className="font-semibold text-sm">Department cost summary</h4>
            <button
              onClick={() => downloadCsv("deskii-department-payroll-summary.csv", buildDepartmentSummaryCsv(departmentSummary))}
              className="inline-flex min-h-9 items-center gap-1.5 rounded-md border border-[var(--border)] px-3 text-xs font-semibold text-[var(--muted)] hover:text-[var(--foreground)] hover:bg-[var(--card-surface)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
            >
            ...
            <table className="w-full text-sm">
              <thead className="bg-[var(--card-surface)] text-[var(--muted)] border-b border-[var(--border)]">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold">Gross</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold">Deductions</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold">Net</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold">Payslips</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border)]">
                {departmentSummary.map((row) => (
                  <tr key={row.department} className="hover:bg-[var(--card-surface)] transition-colors">
                    <td className="px-4 py-3 font-medium">{row.department}</td>
                    <td className="px-4 py-3 text-right">PHP {row.gross.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-red-500">PHP {row.deductions.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-emerald-500 font-semibold">PHP {row.net.toLocaleString()}</td>
                    <td className="px-4 py-3 text-right text-[var(--muted)]">{row.count}</td>
                  </tr>
                ))}
              </tbody>
            </table>
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-[var(--muted)]">
                            {p.employeeDepartment || "Unassigned"} · {p.employeeRole || `ID: ${String(p.employeeId)}`}
                          </div>
                        </div>
                      </div>
                    </td>
...
export default function ReportsTab() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      setLoading(true);
      const queryString = buildPayrollReportSearchParams({
  ...
  }, [reportDepartment, reportStatus, showToastError]);
  ...
  const handleExportSummaryBatch = () => {
    const files = buildPayrollReportBatchFiles({
      stats,
      departmentSummary: latestDepartmentSummary,
      filters: {
        department: reportDepartment,
        status: reportStatus,
      },
    ...
    downloadZip("deskii-payroll-summary-batch.zip", buildReportZipArchive(files));
    showToastSuccess(`Exported ${files.length} report files`);
  };
          ...
          <>
            <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-xl p-4">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
                <div className="flex-1">
                  <label className="block text-xs font-semibold text-[var(--muted)] mb-1" htmlFor="payroll-report-department">
                    Department filter
                  </label>
                  <input
                    ...
                    placeholder="Type exact department, or leave blank"
                ...
                <div className="w-full lg:w-52">
                  <label className="block text-xs font-semibold text-[var(--muted)] mb-1" htmlFor="payroll-report-status">
                    Payslip status
                  </label>
                  <select
                ...
                <Button
                  onClick={() => latestDepartmentSummary.length > 0 && downloadCsv("deskii-payroll-department-summary.csv", buildDepartmentSummaryCsv(latestDepartmentSummary))}
                  variant="outline"
                  size="sm"
                  disabled={latestDepartmentSummary.length === 0}
                  className="min-h-10"
                >
                  <Download className="w-3.5 h-3.5 mr-1.5" />
                  Export summary
                ...
                <Button
                  onClick={handleExportSummaryBatch}
                  variant="outline"
                  size="sm"
            ...
            {!latest ? (
              ...
              <>
                ...
                {latestDepartmentSummary.length > 0 && (
                  <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-xl overflow-hidden">
                    ...
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-[var(--card-surface)] text-[var(--muted)] border-b border-[var(--border)]">
                          <tr>
                            <th className="px-5 py-3 text-left text-xs font-semibold">Department</th>
                            <th className="px-5 py-3 text-right text-xs font-semibold">Gross</th>
                            <th className="px-5 py-3 text-right text-xs font-semibold">Deductions</th>
                            <th className="px-5 py-3 text-right text-xs font-semibold">Net</th>
                            <th className="px-5 py-3 text-right text-xs font-semibold">Payslips</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-[var(--border)]">
                          {latestDepartmentSummary.map((row) => (
                            <tr key={row.department} className="hover:bg-[var(--card-surface)] transition-colors">
                              ...
                              <td className="px-5 py-3.5 text-right text-red-500">PHP {row.deductions.toLocaleString()}</td>
