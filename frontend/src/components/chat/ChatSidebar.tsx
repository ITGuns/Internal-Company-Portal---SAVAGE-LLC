...
import { Archive, ArchiveRestore, Hash, Users, Plus } from 'lucide-react'
import type { Conversation } from '@/lib/chat'
import { getChatSidebarArchiveButtonClass } from '@/lib/chat-sidebar-layout'
...
export default function ChatSidebar({
    ...
    const sidebarActionLabel = showArchived ? 'Restore conversation' : 'Archive conversation'
    const SidebarActionIcon = showArchived ? ArchiveRestore : Archive
    const handleConversationAction = showArchived ? onRestoreConversation : onArchiveConversation
    ...
    return (
        <aside className="w-72 border-r border-[var(--border)] bg-[var(--card-surface)] flex flex-col h-full overflow-hidden" aria-label="Chat conversations">
            <div className="border-b border-[var(--border)] p-3">
                <div className="grid grid-cols-2 gap-2 rounded-lg bg-[var(--background)] p-1" role="tablist" aria-label="Conversation view">
                    <button
                        type="button"
                        role="tab"
                    ...
                    {channels.map(c => {
                        ...
                        return (
                            <div key={c.id} className="relative group">
                                ...
                                >
                                    <Hash className={`w-4 h-4 ${isActive ? 'opacity-100' : 'opacity-40'}`} aria-hidden="true" />
                                    <span className="truncate flex-1">{c.name}</span>
                                    {unreadCounts[c.id] > 0 && !isActive && (
                                ...
                                <button
                                    type="button"
                                    onClick={(e) => { e.stopPropagation(); handleConversationAction(c.id); }}
                                    className={getChatSidebarArchiveButtonClass(isActive)}
                                    aria-label={`${sidebarActionLabel}: ${c.name || 'Channel'}`}
                                    title={sidebarActionLabel}
                                >
                                    <SidebarActionIcon className="w-3.5 h-3.5" aria-hidden="true" />
                                </button>
                            </div>
                        )
                    })}
                    {channels.length === 0 && (
                        <div className="px-3 py-3 text-center text-xs text-[var(--muted)] italic">
                ...
                <div tabIndex={0} aria-label="Direct messages" className="flex-1 overflow-y-auto px-2 pb-4 space-y-1 chat-scroll focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]">
                        ...
                        return (
                            <div key={c.id} className="relative group">
                                ...
                                <button
                                    ...
                                    onClick={(e) => { e.stopPropagation(); handleConversationAction(c.id); }}
                                    className={getChatSidebarArchiveButtonClass(isActive)}
                                    aria-label={`${sidebarActionLabel}: ${other?.name || 'Conversation'}`}
                                    title={sidebarActionLabel}
                                >
                                    <SidebarActionIcon className="w-3.5 h-3.5" aria-hidden="true" />
                                </button>
                    ...
                    {directMessages.length === 0 && (
                        <div className="px-3 py-4 text-center text-xs text-[var(--muted)] italic">
                            {showArchived ? 'No archived private chats' : 'No private chats yet'}
                        </div>
                    )}
                </div>
            </div>
