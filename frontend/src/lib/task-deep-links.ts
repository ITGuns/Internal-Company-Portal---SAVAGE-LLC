...
export type TaskViewPreference = "grid" | "list" | "calendar";
...
const TASK_VIEWS = new Set<TaskViewPreference>(["grid", "list", "calendar"]);
...
export function getTaskViewFromSearch(searchParams: URLSearchParams): TaskViewPreference | null {
  const requestedView = searchParams.get("view")?.trim() as TaskViewPreference | undefined;
  return requestedView && TASK_VIEWS.has(requestedView) ? requestedView : null;
}
