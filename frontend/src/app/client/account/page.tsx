...
import { BriefcaseBusiness, CalendarClock, ExternalLink, ShieldCheck, Users } from "lucide-react";
        ...
        return (
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <BookCallPanel
              ...
              bookingRequests={overview.bookingRequests || []}
...
function BookCallPanel({
  ...
  return (
    <ClientPortalPanel title="Book a Call" icon={CalendarClock}>
      <form
        className="space-y-3"
        onSubmit={async (event) => {
          ...
          setSaving(true);
          try {
            await createClientBookingRequest(organizationId, {
              subject,
            ...
            toast.error(error instanceof Error ? error.message : "Failed to request a call");
          } finally {
            setSaving(false);
          }
        }}
      >
        <FormField id="client-call-subject" label="Subject" value={subject} onChange={setSubject} required />
        <FormField id="client-call-start" label="Preferred Time" type="datetime-local" value={preferredStartAt} onChange={setPreferredStartAt} />
        <textarea
          className="min-h-24 w-full resize-y rounded-md border border-[var(--border)] bg-[var(--card-bg)] px-3 py-2 text-sm text-[var(--foreground)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
          value={notes}
          onChange={(event) => setNotes(event.target.value)}
          placeholder="Agenda or details for the call"
          aria-label="Call request notes"
        />
        <Button type="submit" loading={saving} disabled={!subject.trim()}>Request Call</Button>
      </form>

      {recentRequests.length > 0 ? (
        <div className="mt-4 space-y-2">
          {recentRequests.map((request) => (
            <div key={request.id} className="rounded-[var(--radius-md)] border border-[var(--border)] p-3 text-sm">
              <div className="font-medium">{request.subject}</div>
              <div className="mt-1 text-xs text-[var(--muted)]">
                {getClientPortalOptionLabel(CLIENT_BOOKING_STATUSES, request.status)}
                {request.preferredStartAt ? ` - ${formatClientPortalDate(request.preferredStartAt)}` : ""}
              </div>
            </div>
          ))}
        </div>
