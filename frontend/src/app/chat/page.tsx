...
export default function UnifiedChatPage() {
    ...
    const [conversationView, setConversationView] = useState<ChatArchiveView>('active')
        ...
        async function load() {
            try {
                ...
                const [convData, userData] = await Promise.all([
                    fetchConversations(conversationView).catch(() => {
                        return new Promise<Conversation[]>((resolve) => {
                            setTimeout(async () => {
                                try {
                                    resolve(await fetchConversations(conversationView))
                                } catch {
                                    resolve([])
                                }
                            }, 2000)
                ...
                setSelectedId(current => {
                    if (current && convData.some(conversation => conversation.id === current)) return current
                    const defaultConv = convData.find(c => c.type !== 'direct') || convData[0]
                    return defaultConv?.id || null
                })
            } catch (err) {
    ...
    useEffect(() => {
        ...
        const handleNewConversation = (conv: Conversation) => {
            if (conversationView !== 'active') return
            setConversations(prev => {
    ...
    }, [socket, isConnected, selectedId, currentUser, clearChatBadge, conversationView])
    ...
    const handleArchiveConversation = async (conversationId: string) => {
        ...
        } catch (err) { console.error(err); toast.error("Failed to archive conversation") }
    }
    ...
    const handleRestoreConversation = async (conversationId: string) => {
        ...
        } catch (err) { console.error(err); toast.error("Failed to restore conversation") }
    }
        ...
        <main className="flex flex-col h-[calc(100vh-112px)] overflow-hidden">
            ...
            <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row">
                <ChatSidebar
                    ...
                    showArchived={conversationView === 'archived'}
                    isConnected={isConnected}
                    onSelectConversation={setSelectedId}
                    onArchiveConversation={handleArchiveConversation}
                    onRestoreConversation={handleRestoreConversation}
                    onShowArchivedChange={(showArchived) => setConversationView(showArchived ? 'archived' : 'active')}
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-[var(--muted)] p-8 text-center">
                            ...
                            <p className="max-w-xs text-sm">
                                {conversationView === 'archived'
                                    ? 'Archived conversations stay searchable and can be restored from the sidebar.'
                                    : 'Select a channel or message from the sidebar to start chatting with your team.'}
                            </p>
