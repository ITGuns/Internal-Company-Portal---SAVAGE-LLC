...
test('builds calendar events from non-archived scheduled items', () => {
  ...
  const events = buildClientCalendarEvents([
    { id: 'c1', title: 'Launch prep', status: 'scheduled', projectId: 'project-1', startAt: '2026-06-01T09:00:00.000Z', endAt: '2026-06-01T10:00:00.000Z', channel: 'website', visibleToClient: true },
    { id: 'c2', title: 'Archived draft', status: 'archived', startAt: '2026-06-02T09:00:00.000Z' },
    { id: 'c3', title: 'Missing date', status: 'planned' },
  ], [
    { id: 'project-1', name: 'Website Launch', status: 'in_progress', progress: 72 },
  ]);

  assert.equal(events.length, 1);
  assert.equal(events[0].id, 'c1');
  assert.equal(events[0].title, 'Launch prep · 72%');
  ...
  assert.equal(events[0].extendedProps.projectName, 'Website Launch');
  assert.equal(events[0].extendedProps.projectProgress, 72);
});
...
test('summarizes project progress for calendar and report surfaces', () => {
  const {
    getProjectProgressForCalendarItem,
    summarizeClientProjectProgress,
  } = loadClientPlanningRecords();

  const projects = [
    { id: 'p1', name: 'Launch', status: 'in_progress', progress: 40 },
    { id: 'p2', name: 'Retention', status: 'review', progress: 80 },
    { id: 'p3', name: 'Live Site', status: 'live', progress: 100 },
  ];

  assert.equal(
    getProjectProgressForCalendarItem({ id: 'c1', title: 'Review', status: 'planned', projectId: 'p2' }, projects).name,
    'Retention',
  );

  assert.deepEqual(plain(summarizeClientProjectProgress(projects, [
    { id: 'w1', projectId: 'p1', status: 'completed', progress: 100 },
    { id: 'w2', projectId: 'p2', status: 'in_progress', progress: 25 },
  ])), {
    projectCount: 3,
