...
test('parses task tracking deep-link filters and task ids', () => {
  const { getTaskDeepLinkState, getTaskViewFromSearch } = loadTaskDeepLinkHelper();
  ...
  assert.equal(getTaskViewFromSearch(new URLSearchParams('view=calendar')), 'calendar');
  assert.equal(getTaskViewFromSearch(new URLSearchParams('view=unknown')), null);
});
