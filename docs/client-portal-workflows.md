...
| Admin | Billing - `/operations/clients/billing` | Client billing state and tier-related billing notes. | Set billing status, amount, currency, renewal date, and client visibility. |
...
| Client | Account - `/client/account` | Client account status and access overview. | Review website, service tier, visible billing, and active team members. |
...
## 21. Admin Workflow: Manage Billing Status
...
- The current billing UI does not expose billing notes.
- The backend stores notes and controls whether billing activity is client-visible based on `visibleToClient`.
...
2. Update title, details, channel, status, dates, or visibility.
3. Click `Save Changes`.
