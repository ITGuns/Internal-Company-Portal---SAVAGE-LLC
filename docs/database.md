...
`SchedulerJobRun` stores scheduler execution history for payroll automation and client invoice automation.

- `jobType` currently records `period-advance`, `auto-payslip`, `dept-report`, or `client-invoices`.
- `status`, `triggeredBy`, `startedAt`, `finishedAt`, `durationMs`, `resultJson`, and `errorMsg` support Scheduler tab run history and cron troubleshooting.
- Indexes on `jobType`, `status`, and `startedAt` support recent-run reads and operational filtering.
...
- `ClientStorageRoot` stores the provider-ready storage root for one client organization. The current app-owned flow links to a `FileFolder` so each client can have a dedicated local directory root while future Google Drive, Supabase, or external providers can store external IDs and URLs without changing client ownership.
- `ClientBookingRequest` stores client/admin call requests with provider, preferred times, timezone, meeting URL, notes, and visibility. Client-created requests are limited to manual, requested, client-visible records by the API.
- `ClientPaymentConnection` stores readiness metadata for Stripe, Square, bank account, or manual payment connections. It tracks account type, manual/sandbox/live mode, external customer or merchant identifiers, masked last-four data, webhook status, and internal notes; secrets and API keys are not stored in this table.
- `ClientInvoice` stores manual/provider-linked invoices, invoice numbers, amount/currency, issue/due/paid dates, hosted invoice URL, visibility, and optional relation to `ClientBillingStatus`. Automated monthly invoice generation writes draft manual invoices from due billing status records.
...
- `Participant.archivedAt` stores per-user chat archive state so users can hide conversations without deleting shared history. New messages clear archived state for the conversation participants.
